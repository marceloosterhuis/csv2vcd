...
int main(int argc, char **argv) {
    ...
    struct timeval start, end; // wall-clock timing for perf print
    ...
    FILE *csv, *vcd;           // input CSV, output VCD handles
    char row[MAXCHAR_LINE];    // line buffer reused per row
    char *cols[MAX_COLS];      // pointers into the line buffer for each column

    int row_counter = 0;       // 0=header, 1=initial values, >=2=data rows
    int col;                   // number of columns parsed for the current row
    bool updated;              // flag if any signal changed on this row
    double prev[MAX_COLS];     // last emitted values per signal
    double cur[MAX_COLS];      // current parsed values per signal
